...
int main(int argc, char **argv)
{
	...
	if (i % l.save_steps == 0) {
	    if(l.k > 0){
		MPI_Send(&l.u0[indp(l.ax, l.ay)], 1, t_block, 0, 10, MPI_COMM_WORLD);
	    }
	    else{
		int j;
		for(j = 1; j < l.p; j++){
		    ...
		    if((l.bx - l.ax) == (bx - ax) && (l.by - l.ay) == (by - ay)){
			MPI_Recv(&l.u0[indp(ax, ay)], 1, t_block, j, 10, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		    }
		    else{
			MPI_Datatype t_block2;
			MPI_Type_vector(by - ay + 1, bx - ax + 1, l.nx, MPI_INT, &t_block2);
			MPI_Type_commit(&t_block2);

			MPI_Recv(&l.u0[indp(ax, ay)], 1, t_block2, j, 10, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			MPI_Type_free(&t_block2);
		    }
		}
...
//сообщения вверх-вниз
	...
	MPI_Isend(&l.u0[indp(l.ax, l.ay)], l.bx - l.ax + 1, MPI_INT, kp(l.k - l.px), 0, MPI_COMM_WORLD, &rq1[0]); //отправка вверх
	MPI_Irecv(&l.u0[indp(l.ax, l.by + 1)], l.bx - l.ax + 1, MPI_INT, kp(l.k + l.px), 0, MPI_COMM_WORLD, &rq1[1]); //прием от низа

	MPI_Isend(&l.u0[indp(l.ax, l.by)], l.bx - l.ax + 1, MPI_INT, kp(l.k + l.px), 1, MPI_COMM_WORLD, &rq1[2]); //отправка вниз
	MPI_Irecv(&l.u0[indp(l.ax, l.ay - 1)], l.bx - l.ax + 1, MPI_INT, kp(l.k - l.px), 1, MPI_COMM_WORLD, &rq1[3]); //прием от верха
...
//1 блок send-recv
	...
	MPI_Isend(&l.u0[indp(l.ax, l.ay)], 1, MPI_INT, addressee, 2, MPI_COMM_WORLD, &rq1[4]); //отправка вверх-влево

	addressee = l.k + l.px + 1;
	if(l.by == l.ny - 1){addressee = l.k + l.px + 1 - l.p;}
	if(l.bx == l.nx - 1){addressee = (l.k + 1) % l.p;}
	MPI_Irecv(&l.u0[indp(l.bx + 1, l.by + 1)], 1, MPI_INT, addressee, 2, MPI_COMM_WORLD, &rq1[5]); //прием от низ-право
//2 блок send-recv
	addressee = l.k - l.px + 1;
	if(l.ay == 0){addressee = l.k + 1 - l.px + l.p;}
	if(l.bx == l.nx - 1){addressee = (l.k + 1 - 2 * l.px + l.p) % l.p;}
    	MPI_Isend(&l.u0[indp(l.bx, l.ay)], 1, MPI_INT, addressee, 3, MPI_COMM_WORLD, &rq1[6]); //отправка вверх-вправо

	addressee = l.k + l.px - 1;
	if(l.by == l.ny - 1){addressee = l.k - 1 + l.px - l.p;}
	if(l.ax == 0){addressee = (l.k - 1 + 2 * l.px + l.p) % l.p;}
	MPI_Irecv(&l.u0[indp(l.ax - 1, l.by + 1)], 1, MPI_INT, addressee, 3, MPI_COMM_WORLD, &rq1[7]); //прием от низ-лево
//3 блок send-recv
	addressee = l.k - 1 + l.px;
	if(l.by == l.ny - 1){addressee = l.k - 1 + l.px - l.p;}
	if(l.ax == 0){addressee = (l.k - 1 + 2 * l.px) % l.p;}
	MPI_Isend(&l.u0[indp(l.ax, l.by)], 1, MPI_INT, addressee, 4, MPI_COMM_WORLD, &rq1[8]); //отправка вниз-влево

	addressee = l.k - l.px + 1;
	if(l.ay == 0){addressee = l.k + 1 + l.p - l.px;}
	if(l.bx == l.nx - 1){addressee = (l.k + 1 - 2 * l.px + l.p) % l.p;}
	MPI_Irecv(&l.u0[indp(l.bx + 1, l.ay - 1)], 1, MPI_INT, addressee, 4, MPI_COMM_WORLD, &rq1[9]); //прием от верх-право
//4 блок send-recv

	addressee = l.k + l.px + 1;
	if(l.by == l.ny - 1){addressee = l.k + 1 + l.px - l.p;}
	if(l.bx == l.nx - 1){addressee = (l.k + 1) % l.p;}
	MPI_Isend(&l.u0[indp(l.bx, l.by)], 1, MPI_INT, addressee, 5, MPI_COMM_WORLD, &rq1[10]); //отправка вниз-вправо

	addressee = l.k - l.px - 1;
	if(l.ay == 0){addressee = l.k - 1 - l.px + l.p;}
	if(l.ax == 0){addressee = (l.k - 1 + l.p) % l.p;}
	MPI_Irecv(&l.u0[indp(l.ax - 1, l.ay - 1)], 1, MPI_INT, addressee, 5, MPI_COMM_WORLD, &rq1[11]); //прием от верх-лево

//передача столбцов
        addressee = ((int)(l.k / l.px) == (int)((l.k + 1) / l.px)) ? l.k + 1 : l.k + 1 - l.px;
	MPI_Isend(&l.u0[indp(l.bx, l.ay)], 1, t_column, addressee, 6, MPI_COMM_WORLD, &rq2[0]); //отправка правому
	if(l.k == 0){
	    addressee = l.px - 1;
	}else{
	    addressee = ((int)(l.k / l.px) == (int)((l.k - 1) / l.px)) ? l.k - 1 : l.k - 1 + l.px;
	}
    	MPI_Irecv(&l.u0[indp(l.ax - 1, l.ay)], 1, t_column, addressee, 6, MPI_COMM_WORLD, &rq2[1]); //прием от левого
	...
	MPI_Isend(&l.u0[indp(l.ax, l.ay)], 1, t_column, addressee, 7, MPI_COMM_WORLD, &rq2[2]); //отправка левому
	addressee = ((int)(l.k / l.px) == (int)((l.k + 1) / l.px)) ? l.k + 1 : l.k + 1 - l.px;
	MPI_Irecv(&l.u0[indp(l.bx + 1, l.ay)], 1, t_column, addressee, 7, MPI_COMM_WORLD, &rq2[3]); //прием от правого

	MPI_Waitall(12, rq1, MPI_STATUSES_IGNORE);
    	MPI_Waitall(4, rq2, MPI_STATUSES_IGNORE);
	life_step(&l);
    }
